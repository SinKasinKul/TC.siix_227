...
class Repair
{
  ...
  public function ChkUser($frm)
  {
    $DB = connect_mssql::DB();
    $User = $frm['User'];
    $stmt = "EXEC [STBL_NAL].[dbo].[CHECK_USER] ?";
    $params = array(
  ...
  public function ChkModel($frm)
  {
    ...
    $ItemCD = $frm['ItemCode'];
    $Seiban = $frm['Seiban'];
    $stmt = "EXEC [STBL_NAL].[dbo].[Chk_Model_Seiban] ?, ?";
    $params = array(
  ...
  public function checkInsertLot($frm)
  {
    ...
    $Seiban = $frm['Seiban'];
    $Mode = '10';
    $stmt = "EXEC [STBL_NAL].[dbo].[LOTS_CHK_INSTER] ?, ?, ?, ?, ?, ?, ?";
    $params = array(
  ...
  public function jsonLotQty($frm='')
  {
    ...
    $stmt = "BEGIN
          		...
          		SET @sLot = '$lot';
              SET @sMode = '10';
          			SELECT  [lot_no],[model],[qty] FROM [STBL_NAL].[dbo].[TBL_LOTS]
          			WHERE model = @sModel
          			AND lot_no = @sLot
                AND mode = @sMode
          		END";
  ...
  public function jsonLotSTD($frm='')
  {
    ...
    $stmt = "BEGIN
          		DECLARE @sModel nvarchar(50),
          		        @sLot   nvarchar(20);
          		SET @sModel = '$model';
          			SELECT  [ITEM_DESC],[STD] FROM [STBL_NAL].[dbo].[PSES_STD_PACK]
  ...
  public function JsonMainGrid($frm='')
  {
    $DB = connect_mssql::DB();
    $Barcode = $frm['Barcode'];
    $stmt = "EXEC [STBL_NAL].[dbo].[REPAIR_CHECK_DATA] ?";
    $params = array(
  ...
  public function jsonShowCount($frm='')
  {
    ...
    $stmt = "BEGIN
              ...
              SET @sLot = '$lot';
                ...
                FROM [STBL_NAL].[dbo].[TBL_CHECK_ICT]
  ...
  public function checkInsertBarcode($frm)
  {
    ...
    $stmt = "EXEC [STBL_NAL].[dbo].[REPAIR_INS_DATE] ?, ?, ?, ?, ?, ?, ?, ?, ?";
    $params = array(
      array($Barcode, SQLSRV_PARAM_IN),
      array($Model, SQLSRV_PARAM_IN),
