...
class TranFinal
{
  ...
  public function JsonTranFinal($frm='')
  {
    $DB = connect_mssql::DB();
    $seiban = $frm['seiban'];
    $lot = $frm['lot'];
    $stmt = "BEGIN
              ...
              SELECT ROW_NUMBER() OVER(ORDER BY [id] ASC) AS Row
                ,[Model],[Barcode]
                ,[STATE_SMTA],[STATE_SMTA_DESC]
                ,[STATE_SMTB],[STATE_SMTB_DESC]
                ,[STATE_MN],[STATE_MN_DESC]
                ,[STATE_ICT],[STATE_ICT_DESC]
                ,[STATE_FCT],[STATE_FCT_DESC]
                ,[STATE],[STATE_DESC]
                ,[NHL],[NG],[SHIFT]
                ,CONVERT(varchar,[DATE_UPDATE],120) AS DATE_UPDATE
            FROM [Siix_check_serial].[dbo].[Transtron_FINAL]
  ...
  public function jsonLotQty($frm='')
  {
    ...
    $stmt = "BEGIN
          		...
          		SET @sLot = '$lot';
              SET @sMode = '63';
          			SELECT  lot_no,model,qty FROM TBL_LOTS
          			WHERE model = @sModel
          			AND lot_no = @sLot
                AND mode = @sMode
          		END";
