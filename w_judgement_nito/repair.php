...
class Repair
{
  ...
  public function checkInsertLot($frm)
  {
    ...
    $Seiban = $frm['Seiban'];
    $Mode = '10';
    $stmt = "EXEC [STBL_KOITO].[dbo].[LOTS_CHK_INSTER] ?, ?, ?, ?, ?, ?, ?";
    $params = array(
  ...
  public function jsonLotQty($frm='')
  {
    ...
    $stmt = "BEGIN
          		...
          		SET @sLot = '$lot';
              SET @sMode = '10';
          			SELECT  [lot_no],[model],[qty] FROM [STBL_KOITO].[dbo].[TBL_LOTS]
          			WHERE model = @sModel
          			AND lot_no = @sLot
                AND mode = @sMode
          		END";
  ...
  public function jsonLotSTD($frm='')
  {
    ...
    $stmt = "BEGIN
          		DECLARE @sModel nvarchar(50),
          		        @sLot   nvarchar(20);
          		SET @sModel = '$model';
          			SELECT  [ITEM_DESC],[STD] FROM [STBL_KOITO].[dbo].[PSES_STD_PACK]
  ...
  public function jsonShowCount($frm='')
  {
    ...
    $stmt = "BEGIN
              ...
              SET @sLot = '$lot';
                ...
                FROM [STBL_KOITO].[dbo].[TBL_CHECK_ICT]
  ...
  public function checkInsertBarcode($frm)
  {
    ...
    $stmt = "EXEC [STBL_KOITO].[dbo].[REPAIR_INS_DATE] ?, ?, ?, ?, ?, ?, ?, ?, ?";
    $params = array(
      array($Barcode, SQLSRV_PARAM_IN),
      array($Model, SQLSRV_PARAM_IN),
